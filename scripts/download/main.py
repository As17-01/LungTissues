...
class YandexDiskDownloader:
    ...
    def _download_file(self, path, save_dir):
    ...
    def download(self):
        ...
        for i, file in enumerate(all_images):
            if  self.batch[0] <= i and i < self.batch[1]:
                logger.info(f"File {i + 1}/{len(all_images)}: {file['path']}")
                self._download_file(path=file['path'], save_dir=self.download_location / "images")

        logger.info(f"Loading metadata")
        self._download_file(path="/biospecimen.cart.2024-01-18.json", save_dir=self.download_location)
        self._download_file(path="metadata.cart.2024-01-18.json", save_dir=self.download_location)
